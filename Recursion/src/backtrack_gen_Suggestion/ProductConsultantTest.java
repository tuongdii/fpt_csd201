...
public class ProductConsultantTest {
    public static void main(String[] args) {
        ...
        rfDom.add(new Product("RF04", "Bitachi 380", 9800000));
        ...
        fanDom.add(new Product("F02", "General F02", 7500000));
        fanDom.add(new Product("F03", "Panasonic F80", 10500000));
      
        double budget = 30000000;
        ProductConsultantProblem prob = new ProductConsultantProblem(budget);
        for (int i = 0; i < noOfTVs; i++) {
            prob.addRequest(tvDom);
        }
        for (int i = 0; i < noOfRefrigerators; i++) {
            prob.addRequest(rfDom);
        }
        for (int i = 0; i < noOfFans; i++) {
            prob.addRequest(fanDom);
        }
        prob.solve();
        prob.printSolutions();
    }
